...
export class BookmarkArticle extends Component{
    ...
    componentWillReceiveProps(nextProps){
        if (nextProps.bookmarkedArticleError) {
            this.setState({ bookmarkResultError: nextProps.bookmarkedArticleError, icon_state: 'fas fa-bookmark text-primary'})
        }
        else if(nextProps.bookmarkedArticle){
            this.setState({ bookmarks: nextProps.bookmarkedArticle, icon_state: 'fas fa-bookmark text-primary', bookmarked: true})
        }
        ...
        if (nextProps.bookmarks.results){
            const results = nextProps.bookmarks.results;
            const foundValue = results.filter(bookmark=>bookmark.article.slug===articleSlug);
            if (foundValue.length>0){
                this.setState({ icon_state:'fas fa-bookmark text-primary', bookmarkedArticleId:foundValue[0].id })
            }else{
...
export default connect(mapStateToProps, { bookmarkAction, unBookmark, getBookmarks })( BookmarkArticle);
